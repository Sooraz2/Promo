...
class MenuControlRepository extends Repo
{
    private $table;
    ...
    public function InsertLog($loginUserLog){
    ...
    public function GetAllHiddenMenu(){
        $sql = "SELECT MenuSlug FROM `menu_control` WHERE Moderator = 0";

        $statement = $this->GetDbConnection()->prepare($sql);
        $statement->execute();

        $hiddenMenu = array();
        foreach($statement->fetchAll(\PDO::FETCH_ASSOC) as $menu){
            $hiddenMenu[] = $menu["MenuSlug"];
        }
